...
export default function AIScorePanel() {
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-cyan-400/10 bg-slate-950/55 p-4 transition-colors hover:border-cyan-400/20 sm:p-5">
      <div className="flex items-center gap-2.5">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-cyan-400/15 bg-cyan-400/5">
            ...
            <div className="mt-3 space-y-1.5">
              {aiScore.factors.slice(0, 4).map((factor) => (
                ...
                >
                  ...
                  <span className="min-w-0 truncate">{factor}</span>
                </div>
              ))}
            </div>
          ) : (
