...
export default function DealIntelligencePanel() {
  return (
    <section
      aria-labelledby="deal-intelligence-heading"
      className="min-w-0 overflow-hidden rounded-2xl border border-cyan-500/15 bg-slate-900/70 p-5 shadow-lg shadow-black/10"
    >
      <div className="mb-5">
      ...
      {dealIntelligence.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {dealIntelligence.map((deal) => (
            <article
              key={deal.deal}
              className="flex min-w-0 items-center justify-between gap-4 rounded-xl border border-white/5 bg-slate-950/70 px-4 py-3.5 transition-colors hover:border-cyan-400/20 hover:bg-slate-950"
            >
