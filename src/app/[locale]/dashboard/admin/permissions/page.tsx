import Link from "next/link";


/**
 * ============================================================================
 * ADS CRM — ORGANIZATION ACCESS ADMINISTRATION
 * ============================================================================
 *
 * Route:
 *
 * /[locale]/dashboard/admin/permissions
 *
 * Purpose:
 *
 * Organization scoped access configuration entry point.
 *
 * Boundary:
 *
 * CRM Organization Administration.
 *
 * This is NOT:
 *
 * /admin/permissions
 *
 * which manages ADS platform capabilities.
 *
 * Responsibilities:
 *
 * - Organization access navigation.
 * - Workspace permission configuration entry.
 * - Future organization RBAC extension point.
 *
 * Does NOT:
 *
 * - Create platform permissions.
 * - Modify ADS module capabilities.
 * - Manage global security policies.
 *
 * ============================================================================
 */


interface PermissionArea {

    title:string;

    description:string;

    href:string;

}



const PERMISSION_AREAS:PermissionArea[] = [

    {
        title:
            "Workspace Access",

        description:
            "Manage what organization members can access inside the CRM workspace.",

        href:
            "/dashboard/admin/permissions/workspace",
    },


    {
        title:
            "Module Access",

        description:
            "Configure which enabled CRM modules are available to organization users.",

        href:
            "/dashboard/admin/permissions/modules",
    },


    {
        title:
            "Team Permissions",

        description:
            "Manage access rules across teams and departments.",

        href:
            "/dashboard/admin/permissions/teams",
    },


    {
        title:
            "Access Review",

        description:
            "Review organization access changes and administrative activity.",

        href:
            "/dashboard/admin/permissions/review",
    },

];



function PermissionCard({
    area,
}:{
    area:PermissionArea;
}) {

    return (

        <Link
            href={
                area.href
            }

            className="
    group
    block
    rounded-2xl
    border
    bg-background
    p-5
    transition
    hover:border-primary/40
    hover:bg-muted/30
    focus-visible:outline-none
    focus-visible:ring-2
    focus-visible:ring-primary
"
        >

            <h2
                className="
                    font-semibold
                    text-foreground
                "
            >

                {
                    area.title
                }

            </h2>


            <p
                className="
                    mt-2
                    text-sm
                    leading-6
                    text-muted-foreground
                "
            >

                {
                    area.description
                }

            </p>


            <span
    className="
        mt-4
        inline-flex
        items-center
        gap-1
        text-sm
        font-medium
        text-primary
        transition
        group-hover:translate-x-1
    "
>
    Configure →
</span>
        </Link>

    );

}



export default function OrganizationPermissionsAdminPage(){

    return (

        <main
            className="
                min-w-0
                space-y-8
                px-4
                py-6
                sm:px-6
                lg:px-8
            "
        >

            <header
                className="
                    rounded-2xl
                    border
                    bg-background
                    p-6
                "
            >

                <p
                    className="
                        text-xs
                        font-semibold
                        uppercase
                        tracking-[0.2em]
                        text-primary
                    "
                >
                    Organization Administration
                </p>


                <h1
                    className="
                        mt-2
                        text-3xl
                        font-bold
                    "
                >
                    Permissions
                </h1>


                <p
                    className="
                        mt-3
                        max-w-3xl
                        text-sm
                        leading-6
                        text-muted-foreground
                    "
                >
                    Configure organization-level access,
                    workspace controls and module availability.
                </p>

            </header>



            <section
                className="
                    grid
                    gap-5
                    md:grid-cols-2
                "
            >

                {
                    PERMISSION_AREAS.map(
                        area => (

                            <PermissionCard
                                key={
                                    area.href
                                }
                                area={
                                    area
                                }
                            />

                        ),
                    )
                }

            </section>


        </main>

    );

}